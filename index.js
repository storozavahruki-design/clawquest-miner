...
// ==========================================
// ОСНОВНАЯ ЛОГИКА ТАНКА «ДЕМОН» (onTick)
// ==========================================
function onTick(ctx) {
  ...
  // 2. АТАКА (с предсказанием и проверкой стен)
  if (enemy.tank && !me.status.fireLocked) {
    if (myX === enemyX || myY === enemyY) {
      const isHorizontal = myY === enemyY;
      const targetDir = isHorizontal 
        ? (myX > enemyX ? "left" : "right") 
        : (myY > enemyY ? "up" : "down");
      
      let wallBetween = false;
      if (isHorizontal) {
        for (let x = Math.min(myX, enemyX); x <= Math.max(myX, enemyX); x++) {
          if (game.map[x] && game.map[x][myY] === "x") wallBetween = true;
        }
      } else {
        for (let y = Math.min(myY, enemyY); y <= Math.max(myY, enemyY); y++) {
          if (game.map[myX] && game.map[myX][y] === "x") wallBetween = true;
        }
      }

      if (!wallBetween) {
        if (me.tank.direction !== targetDir) {
          me.turn(targetDir);
        } else {
          me.fire();
          return;
  ...
  // 3. ПРИОРИТЕТЫ ДВИЖЕНИЯ
  let targetX = null;
  let targetY = null;

  const distToStar = (starX !== null) ? (Math.abs(myX - starX) + Math.abs(myY - starY)) : 999;
  
  if (starX !== null && (distToStar < 5 || me.stars <= enemy.stars)) {
    targetX = starX;
    targetY = starY;
  } else if (enemyX !== null) {
    targetX = enemyX;
  ...
  // 4. ДВИЖЕНИЕ С ОБХОДОМ СТЕН
  if (targetX !== null) {
    let moveDir = null;
    if (myX < targetX) moveDir = "right";
    else if (myX > targetX) moveDir = "left";
    else if (myY < targetY) moveDir = "down";
    else if (myY > targetY) moveDir = "up";

    let canMove = true;
    if (moveDir === "right" && (!game.map[myX + 1] || game.map[myX + 1][myY] === "x" || game.map[myX + 1][myY] === "m")) canMove = false;
    if (moveDir === "left" && (!game.map[myX - 1] || game.map[myX - 1][myY] === "x" || game.map[myX - 1][myY] === "m")) canMove = false;
    if (moveDir === "down" && (!game.map[myX] || game.map[myX][myY + 1] === "x" || game.map[myX][myY + 1] === "m")) canMove = false;
    if (moveDir === "up" && (!game.map[myX] || game.map[myX][myY - 1] === "x" || game.map[myX][myY - 1] === "m")) canMove = false;

    if (canMove) {
      if (me.tank.direction !== moveDir) {
        me.turn(moveDir);
      } else {
        me.go();
      }
...
// Экспортируем функцию танка, если потребуется подключать из других модулей
export { onTick };
